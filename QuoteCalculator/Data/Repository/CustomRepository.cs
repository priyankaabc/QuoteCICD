...
namespace QuoteCalculator.Data.Repository
{
    ...
    public class CustomRepository
    {
        public static List<GetCity_Result> GetCascadingCityList(string CountryId)
        {
            ...
            {
                dataList = context.GetCity(CountryId, SessionHelper.COMPANY_ID).ToList();
            }
        ...
        public static List<SP_GetQuoteContents_Result> GetQuoteContents(string fromCountryCode, string countrycode, string quotetype)
        {
            using (quotesEntities dbContext = BaseContext.GetDbContext())
            {
                List<SP_GetQuoteContents_Result> list = dbContext.SP_GetQuoteContents(fromCountryCode, countrycode, quotetype, SessionHelper.COMPANY_ID).ToList();
                return list;
            }
        }
        ...
        public static int GetNextCustomerRefQuoteNo(string moveType, long customerRefNo, long quoteId)
        {
            using (quotesEntities dbContext = BaseContext.GetDbContext())
            {
                string sqlQuery = "SELECT [dbo].[FN_GetNextCustomerRefQuoteNo] ('" + moveType  + "', '" + customerRefNo + "','" + quoteId + "')";
                quotesEntities entityObj = new quotesEntities();
                int customerQuoteNo = entityObj.Database.SqlQuery<int>(sqlQuery).FirstOrDefault();
